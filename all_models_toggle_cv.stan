functions {
  vector choiceprob(real rho, real kappa, real sigma, real c, int include_scalar, int include_constant, vector v_l, vector rew_multi, vector p_l, vector v_s) {
    ...
    for (i in 1:rows(v_l)){
      ...
      */
      if (include_scalar) {
        if (include_constant){ /* scalar and constant noise */
        noise_term =  sqrt( (u_l * sigma + c)^2 + (u_s * sigma + c)^2 );
        } else{ /* scalar, no constant noise */
...
data {
  ...
  // below is data from the heldout portion
  int<lower=0> D; // number of trial types in test data
  vector[D] test_lottery_mag; // Lottery value for that choice
  vector[D] test_lottery_prob; // Lottery probabilities for that choice
  vector[D] test_sb_mag; // Surebet values for that choice
  vector[D] test_total_rew_multi; // total reward multiplier = base_reward * rew_multi
  vector[D] test_prev_outcome; // -1 = lose, 1 = win, 0 = surebet
  vector[D] test_prev_choice; // Previous choice. 0 = surebet, 1 = lottery, -1 = "other" (violation/first trial)
  int test_n_chose_lott[D]; // number chose lottery for this trial type
  int test_n_trials[D]; // total number of trials for this trial type
  /* Hyperparameters */
  real<lower=0> rho_mu;
  ...
  int<lower=0,upper=1> include_constant; // Boolean.
...
parameters {
  ...
  real<lower=0> sigma; /* noise multiplicative scaler */
  vector[include_constant] c_raw; /* noise additive constant */
...
model {
  ...
  n_chose_lott ~ binomial(n_trials, mixprob(choiceprob(rho, kappa, sigma, c, include_scalar, include_constant, 
...
generated quantities {
  vector[D] log_lik;
  vector[D] p_chose_lott;
  
  p_chose_lott = mixprob(choiceprob(rho, kappa, sigma, c, include_scalar, include_constant, 
                                    test_lottery_mag, test_total_rew_multi, test_lottery_prob, test_sb_mag),
                        omega,
                        (include_history ? omega_win : omega),
                        (include_history ? omega_lose : omega),
                        test_prev_outcome, test_prev_choice);
  
  for (d in 1:D)
  log_lik[d] = binomial_lpmf(test_n_chose_lott[d] | test_n_trials[d], p_chose_lott[d]);
}
