compute_reward = function(df){
  base_reward = as.double(sort(unique(df$reward))[2])
  high_reward = as.double(sort(unique(df$reward))[3])
...
plot_rho_sigma_tradeoff = function(n_trials = 1000, sigmas = 1, rewmulti = 5, sim = FALSE){
    ...
    for (this_rho in rhos){
      for (this_sigma in sigmas){
        this_df = sim_subject(n_trials = n_trials,
                              params_list = list(sigma = this_sigma, rho = this_rho, omega_1 = 1, omega_2 = 0, omega_3 = 0), df = df)
        this_reward = sum(this_df$reward)
        temp_df = data.frame(sigma_s = sigma_s, rew_multi = rew_multi, sigma = this_sigma, rho = this_rho, reward = this_reward)
        reward_df = rbind(reward_df, temp_df)
      }
    }
    write.csv(reward_df, file = 'csv/rho_sigma_tradeoff.csv')
  } else{
    reward_df = read.csv('csv/all_tradeoff.csv') %>% filter(sigma_s == sigmas, rew_multi == rewmulti)
  }
  # get subjects fits
  fit_df = read.csv('csv/bdt_fits.csv')
  fit_df = fit_df %>% group_by(subjid) %>% mutate(max_lp = which.max(lp__)) %>% 
  ...
  p = ggplot() + theme_classic(BASE_SIZE) +
    geom_tile(reward_df, mapping = aes(x = sigma, y = rho, fill = norm_reward), color = NA) +
    geom_point(fit_df, mapping = aes(x = sigma, y = rho, color = as.factor(subjid)), shape = 15, size = 7) +
    geom_point(reward_df2, mapping = aes(x = sigma, y = best_rho), color = 'indianred', shape = 0, size = 7) +
    xlab(TeX(latex_params[['sigma']])) + ylab(TeX(latex_params[['rho']])) +
  ...
  scale_save(p, sprintf('tradeoff_%.1f_%.1f', sigmas, rewmulti), 20, 15)
  return(p)
}