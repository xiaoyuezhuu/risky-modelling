...
data {
  ...
  vector[P] pred_prev_outcome; // 1 = lottery win, -1 = lottery lose, 0 = surebet
  vector[P] pred_prev_choice; // Previous choice. 0 = surebet, 1 = lottery, -1 = "other" (violation/first trial)
...
generated quantities {
  vector[P] p_chose_lott;
  vector[P] pred_n_chose_lott;
  
  p_chose_lott = mixprob(choiceprob(rho, kappa, sigma, include_scalar, 
  pred_lottery_mag, pred_total_rew_multi, pred_lottery_prob, pred_sb_mag),
  omega,
  (include_history ? omega_win : omega),
  (include_history ? omega_lose : omega),
  pred_prev_outcome, pred_prev_choice);
  for (p in 1:P){
    pred_n_chose_lott[p] = binomial_rng(pred_n_trials[p], p_chose_lott[p]);
  }
}
